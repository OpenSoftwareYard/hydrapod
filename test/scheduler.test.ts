...
describe("getNextAvailableIP", () => {
  ...
  it("should throw an error for invalid CIDR notation (invalid prefix)", () => {
    ...
    expect(() => getNextAvailableAddress("192.168.1.0/33", usedIPs)).toThrow(
      "Invalid CIDR prefix length",
    );
    expect(() => getNextAvailableAddress("192.168.1.0/-1", usedIPs)).toThrow(
      "Invalid CIDR prefix length",
    );
    expect(() => getNextAvailableAddress("192.168.1.0/abc", usedIPs)).toThrow(
      "Invalid CIDR prefix length",
    );
  });
  ...
  it("should throw an error for invalid IP address format", () => {
    const usedIPs = new Set(["192.168.1.1"]);
    expect(() => getNextAvailableAddress("192.168.1/24", usedIPs)).toThrow(
      "Invalid IP address format",
    );
    expect(() => getNextAvailableAddress("192.168.1.300/24", usedIPs)).toThrow(
      "Invalid IP address format",
    );
    expect(() => getNextAvailableAddress("192.168.1.a/24", usedIPs)).toThrow(
      "Invalid IP address format",
    );
  });
